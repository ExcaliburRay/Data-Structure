...
public class RedBlackTree {
     ...
     */
    public RedBlackTree() {
        nil = new RedBlackNode("-1",BLACK,null,null,null);
        root = new RedBlackNode("-1",BLACK,nil,nil,nil);
        recentCompares = 1;
    }
    ...
    public void insert(String value){
        ...
        while(child!=nil){
            parent = child;
            if(value.compareTo(child.getData())<0){
                child = child.getLc();
            }else{
                child = child.getRc();
            }
        ...
        if(parent.getData() == nil.getData()){
            root = new RedBlackNode(value, BLACK, nil,nil,nil);
        }else{
            if(value.compareTo(parent.getData())<=0){
                RedBlackNode left = new RedBlackNode(value,RED,parent,nil,nil);
                parent.setLc(left);
                RBInsertFixup(parent.getLc());
            }else{
                RedBlackNode right = new RedBlackNode(value,RED,parent,nil,nil);
                parent.setRc(right);
